...
class VueJsLayoutPublishCommand extends PublishBaseCommand
{
     ...
     * @var string
     */
    protected $name = 'artomator.publish:vuejs';
    ...
    {
        $viewsPath = config('pwweb.artomator.path.views', base_path('resources/views/'));
        $resourcesPath = config('pwweb.artomator.path.resourcesPath', base_path('resources/'));
        $vendorPath = $resourcesPath.'assets/vendor/';
        $assetsJsPath = config('pwweb.artomator.path.assetsJsPath', base_path('resources/assets/js/'));
        $assetsCssPath = config('pwweb.artomator.path.assetsCssPath', base_path('resources/assets/css/'));
        $templateType = config('pwweb.artomator.templates', 'core-templates');
        $requestPath = config('pwweb.artomator.path.api_request', base_path('app/Http/Requests/'));
    ...
    {
        $path = config('pwweb.artomator.path.routes', app_path('Http/routes.php'));
        $routeContents = file_get_contents($path);

        $routesTemplate = get_template('routes.auth', 'artomator');
        if ($this->laravelVersion == '5.1') {
    ...
    {
        $templateData = get_template('home_controller', 'artomator');

        $templateData = $this->fillTemplate($templateData);

        $controllerPath = config('pwweb.artomator.path.controller', app_path('Http/Controllers/'));
    ...
    private function fillTemplate($templateData)
    {
        $templateData = str_replace(
            ...
            config('pwweb.artomator.namespace.controller'), $templateData
        );

        $templateData = str_replace(
            '$NAMESPACE_REQUEST$',
            config('pwweb.artomator.namespace.request'), $templateData
        );
