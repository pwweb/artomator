...
class GeneratorPublishCommand extends PublishBaseCommand
{
    ...
    {
        $apiVersion = config('pwweb.artomator.api_version', 'v1');
        $apiPrefix = config('pwweb.artomator.api_prefix', 'api');
    ...
    {
        $testsPath = config('pwweb.artomator.path.tests', base_path('tests/'));
        $testsNameSpace = config('pwweb.artomator.namespace.tests', 'Tests');
        ...
        $testAPIsPath = config('pwweb.artomator.path.api_test', base_path('tests/APIs/'));
        if (!file_exists($testAPIsPath)) {
            FileUtil::createDirectoryIfNotExist($testAPIsPath);
            $this->info('APIs Tests directory created');
        }

        $testRepositoriesPath = config('pwweb.artomator.path.repository_test', base_path('tests/Repositories/'));
    ...
    {
        $templateData = get_template('app_base_controller', 'artomator');
    ...
    {
        $templateData = get_template('base_repository', 'artomator');
