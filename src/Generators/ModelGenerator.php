...
use PWWEB\Artomator\Common\GeneratorFieldRelation;
use PWWEB\Artomator\Utils\FileUtil;
use PWWEB\Artomator\Utils\TableFieldsGenerator;
...
class ModelGenerator extends BaseGenerator
{
    ...
    {
        $templateData = get_template('model.model', 'artomator');
    ...
    private function fillTemplate($templateData)
    {
        ...
        if ($this->commandData->getOption('primary')) {
            $primary = arty_tab()."protected \$primaryKey = '".$this->commandData->getOption('primary')."';\n";
        } else {
            $primary = '';
        }
        ...
        $templateData = str_replace('$FIELDS$', implode(','.arty_nl_tab(1, 2), $fillables), $templateData);

        $templateData = str_replace('$RULES$', implode(','.arty_nl_tab(1, 2), $this->generateRules()), $templateData);

        $templateData = str_replace('$CAST$', implode(','.arty_nl_tab(1, 2), $this->generateCasts()), $templateData);

        $templateData = str_replace(
            '$RELATIONS$',
            fill_template($this->commandData->dynamicVars, implode(PHP_EOL.arty_nl_tab(1, 1), $this->generateRelations())),
            $templateData
        );
    ...
    private function fillSoftDeletes($templateData)
    {
        if (!$this->commandData->getOption('softDelete')) {
            ...
            $templateData = str_replace('$SOFT_DELETE$', arty_tab()."use SoftDeletes;\n", $templateData);
            $deletedAtTimestamp = config('pwweb.artomator.timestamps.deleted_at', 'deleted_at');
            $templateData = str_replace(
                '$SOFT_DELETE_DATES$', arty_nl_tab()."protected \$dates = ['".$deletedAtTimestamp."'];\n",
                $templateData
            );
        }
    ...
    private function fillDocs($templateData)
    {
        if ($this->commandData->getAddOn('swagger')) {
            ...
            $docsTemplate = get_template('docs.model', 'artomator');
    ...
    private function fillTimestamps($templateData)
    {
        ...
        if ($this->commandData->getOption('fromTable')) {
            if (empty($timestamps)) {
                $replace = arty_nl_tab()."public \$timestamps = false;\n";
            } else {
                list($created_at, $updated_at) = collect($timestamps)->map(function ($field) {
                    return !empty($field) ? "'$field'" : 'null';
                });

                $replace .= arty_nl_tab()."const CREATED_AT = $created_at;";
                $replace .= arty_nl_tab()."const UPDATED_AT = $updated_at;\n";
            }
        }
    ...
    {
        $dont_require_fields = config('pwweb.artomator.options.hidden_fields', [])
                + config('pwweb.artomator.options.excluded_fields', []);
