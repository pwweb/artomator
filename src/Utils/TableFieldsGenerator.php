...
class TableFieldsGenerator
{
    ...
    public function __construct($tableName, $ignoredFields)
    {
        ...
        $mappings = config('pwweb.artomator.from_table.doctrine_mappings', []);
        ...
        $this->defaultSearchable = config('pwweb.artomator.options.tables_searchable_default', false);
    }
    ...
    {
        if (!config('pwweb.artomator.timestamps.enabled', true)) {
            return [];
        }

        $createdAtName = config('pwweb.artomator.timestamps.created_at', 'created_at');
        $updatedAtName = config('pwweb.artomator.timestamps.updated_at', 'updated_at');
        $deletedAtName = config('pwweb.artomator.timestamps.deleted_at', 'deleted_at');
