...
class ArtomatorServiceProvider extends ServiceProvider
{
    ...
    public function register()
    {
        ...
        $this->app->singleton(
            'artomator.graphql', function ($app) {
                return new GraphQLGeneratorCommand();
            }
        );

        $this->app->singleton(
            'artomator.graphql_scaffold', function ($app) {
                return new GraphQLScaffoldGeneratorCommand();
            }
        );

        $this->app->singleton(
            'artomator.graphql.query', function ($app) {
                return new GraphQLQueryGeneratorCommand();
            }
        );

        $this->app->singleton(
            'artomator.graphql.mutations', function ($app) {
                return new GraphQLMutationsGeneratorCommand();
            }
        );

        $this->app->singleton(
            'artomator.graphql.type', function ($app) {
