...
class GeneratorConfig
{
    ...
    public function loadNamespaces(CommandData &$commandData)
    {
        ...
        $this->nsRepository = config('pwweb.artomator.namespace.repository', 'App\Repositories').$prefix;
        $this->nsModel = config('pwweb.artomator.namespace.model', 'App\Models').$prefix;
        if (config('pwweb.artomator.ignore_model_prefix', false)) {
            $this->nsModel = config('pwweb.artomator.namespace.model', 'App\Models');
        }
        $this->nsDataTables = config('pwweb.artomator.namespace.datatables', 'App\DataTables').$prefix;
        $this->nsModelExtend = config(
            'pwweb.artomator.model_extend_class',
    ...
    public function loadPaths()
    {
        ...
        $this->pathRepository = config(
            'pwweb.artomator.path.repository',
            app_path('Repositories/')
        ).$prefix;

        $this->pathModel = config('pwweb.artomator.path.model', app_path('Models/')).$prefix;
        if (config('pwweb.artomator.ignore_model_prefix', false)) {
            $this->pathModel = config('pwweb.artomator.path.model', app_path('Models/'));
        }
        ...
        $this->pathRequest = config('pwweb.artomator.path.request', app_path('Http/Requests/')).$prefix;

        $this->pathRoutes = config('pwweb.artomator.path.routes', base_path('routes/web.php'));
        $this->pathFactory = config('pwweb.artomator.path.factory', database_path('factories/')).$prefix;

        $this->pathViews = config(
            'pwweb.artomator.path.views',
            resource_path('views/')
        ).$viewPrefix.$this->mSnakePlural.'/';

        $this->pathSeeder = config('pwweb.artomator.path.seeder', database_path('seeds/')).$prefix;
        $this->pathDatabaseSeeder = config('pwweb.artomator.path.database_seeder', database_path('seeds/DatabaseSeeder.php'));

        $this->modelJsPath = config(
            'pwweb.artomator.path.modelsJs',
    ...
    public function loadDynamicVariables(CommandData &$commandData)
    {
        ...
        $commandData->addDynamicVariable(
            '$API_PREFIX$',
            config('pwweb.artomator.api_prefix', 'api')
        );

        $commandData->addDynamicVariable(
            '$API_VERSION$',
            config('pwweb.artomator.api_version', 'v1')
        );
    ...
    public function prepareOptions(CommandData &$commandData)
    {
        ...
        if (empty($this->options['save'])) {
            $this->options['save'] = config('pwweb.artomator.options.save_schema_file', true);
        }

        $this->options['softDelete'] = config('pwweb.artomator.options.softDelete', false);
        $this->options['repositoryPattern'] = config('pwweb.artomator.options.repository_pattern', true);
    ...
    {
        $this->prefixes['route'] = explode('/', config('pwweb.artomator.prefixes.route', ''));
        $this->prefixes['path'] = explode('/', config('pwweb.artomator.prefixes.path', ''));
        $this->prefixes['view'] = explode('.', config('pwweb.artomator.prefixes.view', ''));
    ...
    {
        $this->addOns['swagger'] = config('pwweb.artomator.add_on.swagger', false);
        $this->addOns['tests'] = config('pwweb.artomator.add_on.tests', false);
        $this->addOns['datatables'] = config('pwweb.artomator.add_on.datatables', false);
        $this->addOns['menu.enabled'] = config('pwweb.artomator.add_on.menu.enabled', false);
        $this->addOns['menu.menu_file'] = config('pwweb.artomator.add_on.menu.menu_file', 'layouts.menu');
    }
}
